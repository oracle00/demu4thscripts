...
class App(tk.Tk):
    ...
    def validate_overwrite_text(self, event=None):
        ...
        if not text_content:
            self.overwrite_data = None
            self.overwrite_status_var.set("ファイルを選択してください")
        ...
        try:
            ...
            self.overwrite_data = None
            self.overwrite_status_var.set("無効: 正しい16進数フォーマットではありません")
            self.btn_overwrite.config(state="disabled")
            return

        if len(data) != CARD_SIZE:
            self.overwrite_data = None
            self.overwrite_status_var.set(f"無効: サイズが {CARD_SIZE} バイトではありません")
            self.btn_overwrite.config(state="disabled")
            return
